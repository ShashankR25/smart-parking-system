...
void setup()
{
    ...
    lcd.backlight();
    pinMode(Entry, INPUT);
    DEPARTMENT OF COMPUTER SCIENCE, BCA[IoT] PAGE 18 SMART PARKING SYSTEM National College Jayanagar
                                        pinMode(Exit, INPUT);
    pinMode(IR1, INPUT);
    pinMode(IR2, INPUT);
    pinMode(IR3, INPUT);
...
void loop()
{
    ...
    if (digitalRead(Entry) == LOW && flag1 == 0)
    {
        if (Slot > 0)
        {
            flag1 = 1;
        ...
        {
            lcd.setCursor(0, 0);
            DEPARTMENT OF COMPUTER SCIENCE, BCA[IoT] PAGE 19 SMART PARKING SYSTEM National College Jayanagar
                                                lcd.print(" SORRY :( ");
            lcd.setCursor(0, 1);
